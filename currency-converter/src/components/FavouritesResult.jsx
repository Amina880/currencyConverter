...
function FavouritesResult() {
  ...
  return (
    ...
    {currencyData.length > 0 ? (
        <div className='w-11/12 sm:w-8/12 mx-auto justify-center bg-white dark:bg-slate-400 rounded-2xl px-3 sm:p-8 pt-4 flex-grow sm:flex-grow-0 mt-3 pb-5 sm:pb-20 grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6'>
        {currencyData.map((currencyD, index) => (
            <Link 
            to={`/results?s=${currencyD.base_code}/${currencyD.target_code}/${currencyD.amount}`}
            key={index}
            >
                <div className='border-2 border-slate-200 rounded-md sm:rounded-lg hover:shadow-xl hover:scale-105 dark:bg-slate-200 dark:text-gray-700  ' key={index}> 
